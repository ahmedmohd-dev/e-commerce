...
const generateReceiptPDF = async (order) => {
  ...
  doc.setFontSize(10);
  doc.setFont('helvetica', 'normal');
  doc.text(`Name: ${order.shippingAddress.firstName} ${order.shippingAddress.lastName}`, 20, 90);
  ...
  doc.text(`${order.shippingAddress.city}, ${order.shippingAddress.state}`, 20, 137);
  doc.text(`${order.shippingAddress.zipCode}, ${order.shippingAddress.country}`, 20, 144);
  ...
  if (order.paymentDetails?.transactionId) {
    doc.text(`Transaction ID: ${order.paymentDetails.transactionId}`, 20, yPosition + 27);
  }
...
const downloadReceipt = async (req, res) => {
  try {
    ...
    res.setHeader('Content-Disposition', `attachment; filename="receipt-${order._id.slice(-8)}.pdf"`);
